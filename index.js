...
// Sistema de database persistente
const DATABASE_FILE = 'posts_database.json';

// Função para carregar dados do database
...
// Função para salvar dados no database
function saveDatabase() {
  try {
    const data = {
      posts: Object.fromEntries(postLikes.entries()),
      likes: Object.fromEntries(
        Array.from(postLikes.entries()).map(([postId, likes]) => [postId, Array.from(likes)])
      ),
      comments: Object.fromEntries(postComments.entries()),
      authors: Object.fromEntries(postAuthors.entries()),
      privacy: Object.fromEntries(postPrivacySettings.entries()),
      commentCounts: Object.fromEntries(
        Array.from(userCommentCount.entries()).map(([postId, counts]) => [
          postId, Object.fromEntries(counts.entries())
        ])
      )
    };
...
// Carregar dados do database na inicialização
const dbData = loadDatabase();

// Maps para armazenar dados dos posts (agora com dados persistentes)
const postLikes = new Map(
  Object.entries(dbData.likes).map(([postId, likes]) => [postId, new Set(likes)])
);
const postComments = new Map(Object.entries(dbData.comments));
const postAuthors = new Map(Object.entries(dbData.authors));
const postPrivacySettings = new Map(Object.entries(dbData.privacy));
const userCommentCount = new Map(
  Object.entries(dbData.commentCounts).map(([postId, counts]) => [
    postId, new Map(Object.entries(counts))
  ])
);

// Salvar database automaticamente a cada 30 segundos
setInterval(saveDatabase, 30000);

console.log(`Database carregado: ${Object.keys(dbData.authors).length} posts encontrados`);
...
client.on('messageCreate', async message => {
  ...
  if (message.channel.id === '1392228130361708645' && 
    ...
    try {
      ...
      // Armazenar o ID da mensagem do webhook para referência futura
      postAuthors.set(postId, message.author.id);
      ...
      // Se houver erro, limpar os dados do post
...
`)
  ...
  if (customId.startsWith('like_')) {
    ...
    if (!postLikes.has(postId)) {
      ...
      console.log('Posts disponíveis:', Array.from(postLikes.keys()));
